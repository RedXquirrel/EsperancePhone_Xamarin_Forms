...
{
    public partial class EsperancePhoneApiManager
    {
        public static bool CreateCurrentUserOnInstantiation { get; set; }

        static EsperancePhoneApiManager defaultInstance = new EsperancePhoneApiManager();
        ...
        private EsperancePhoneApiManager()
        {
            client = new MobileServiceClient(
                Constants.ApplicationURL);

            if (CreateCurrentUserOnInstantiation)
            {
                CreateCurrentUser();
            }
        ...
        private static void CreateCurrentUser()
        {
            ...
            {
                Settings.MobileServiceAuthenticationToken = client.CurrentUser.MobileServiceAuthenticationToken;
            }

            if (client.CurrentUser == null && !string.IsNullOrEmpty(Settings.UserId) &&
                !string.IsNullOrEmpty(Settings.MobileServiceAuthenticationToken))
            {
                client.CurrentUser = new MobileServiceUser(Settings.UserId);
                client.CurrentUser.MobileServiceAuthenticationToken = Settings.MobileServiceAuthenticationToken;
            }
        }

        public static EsperancePhoneApiManager DefaultManager
        {
            get
            {
                CreateCurrentUser();
